...
// Schema for the demo submission form
const demoSchema = z.object({
  artistName: z.string().min(2, "Artist name must be at least 2 characters.").max(100),
  socialLinks: z.string().max(500, "Social links section too long.").optional().nullable(), // Using textarea for multiple links
  ...
  demoFile: z.instanceof(File).refine(file => file.size > 0, 'Demo track file is required.')
                         .refine(file => file.size <= 50 * 1024 * 1024, 'Demo track must be 50MB or less.') // 50MB limit for demo
...
export function SubmitDemoForm({ onSuccess, onCancel, className }: SubmitDemoFormProps) {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit, (errors) => console.log("Demo Form Errors:", errors))} className={cn("space-y-4", className)}>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Your stage name or band name" {...field} disabled={isSubmitting} className="focus:ring-accent" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Title of your demo track" {...field} disabled={isSubmitting} className="focus:ring-accent" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
            ...
            render={({ fieldState }) => (
                ...
                <FormControl>
                    ...
                    {fileName ? (
                        ...
                        <Button type="button" variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-destructive rounded-full" onClick={clearFile} aria-label="Remove MP3 file" disabled={isSubmitting}>
                            <X className="h-4 w-4" />
                        </Button>
                        </div>
                    ) : (
                        ...
                        <div className="flex text-sm text-muted-foreground justify-center">
                            ...
                            <input id="demoFile" name="demoFile" type="file" className="sr-only" accept="audio/mpeg" onChange={handleFileChange} ref={fileInputRef} disabled={isSubmitting} />
                            </label>
